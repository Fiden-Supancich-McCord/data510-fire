...
| Global Biodiversity Information Facility (GBIF) | Species occurrence records for biodiversity/vegetation metrics | Open-data framework using CC0, CC BY, and CC BY-NC licenses | Oregon, 2015-2025 | Tabular with coordinates | 23,571 records; ~14.6 MB |
...
![Species abundance by Indigenous land designation status.](/images/species_abundance_scatterplot.png){#fig-species-abundance width=80%}
...
::: {#fig-species layout-ncol=2}
![Residuals vs fitted.](/images/species_residvfitted_plot.png){#fig-species-resid-fitted}

![Q-Q residuals.](/images/species_qq_plot.png){#fig-species-qq}

![Scale location.](/images/species_scalelocation_plot.png){#fig-species-scalelocation}

![Cook’s distance.](/images/species_cooks_plot.png){#fig-species-cooks}

Forest compositional diversity measures.
:::
...
ggsave("images/n_fires_map_final.png", n_fires_map, width=5, height=3)
```
...
gdf %>%
    ...
    labs(
        ...
        title="Maximum fire duration by land type",
        caption="n = 4261\nSource: InFORM, IFPH, LandMark"
    ) +
    theme(axis.text=element_text(size=10))
...
#| fig-cap: "Fire size distribution by prescribed burn history and Indigenous land designation status in 2025."
#| echo: false
#| fig-width: 5
#| fig-height: 5
...
rx_indigenous_size_2025 <- tgdf %>%
  ...
  labs(
    ...
    y="Percent of cells",
    title="Fire size distribution by prescribed burn history\nand Indigenous land designation in 2025",
    fill="",
    caption='n = 1558\nSource: IFPH, InFORM'
  ) +
  theme_bw() +
...
rx_indigenous_size_2025

ggsave("images/rx_indigenous_size_2025_final.png", rx_indigenous_size_2025, width=5, height=5)
```
...
rx_annual_size_class

#ggsave("images/rx_annual_size_class_final.png", rx_annual_size_class, width=5, height=4)
```
...
tgdf %>% 
    ...
    scale_y_continuous(trans=scales::pseudo_log_trans(base = 10), labels=scales::comma) +
    labs(
        title="Fire size by cell wildfire history",
        x="Wildfire history",
        y="Maximum acres burned",
        caption="Source: IFPH, InFORM"
    ) +
...
![SHAP values for a Random Forest classifier with prescribed burn, Indigenous designation, and plant sum.](/images/shap_summary_ndgn_plnt_rx.png){#fig-shap-summary width=70%}
...
![SHAP values for Random Forest classifier with added wildfire metrics.](/images/shap_summary_ndgn_plnt_rx_wf.png){#fig-shap-summary-wf width=70%}
...
![SHAP values for the Random Forest classifier with all plant species counts included separately.](/images/shap_summary_ndgn_plnt_rx_wf_p.png){#fig-shap-summary-plant width=80%}
...
![Confusion matrix for the Random Forest classifier.](/images/conf_matrix_ndgn_plnt_rx.png){#fig-conf-matrix width=60%}
...
![Confusion matrix for the Random Forest classifier with wildfire metrics.](/images/conf_matrix_ndgn_plnt_rx_wf.png){#fig-conf-matrix width=60%}
...
![Confusion matrix for the Random Forest classifier with all plant species counts included separately.](/images/conf_matrix_ndgn_plnt_rx_wf_p.png){#fig-conf-matrix-plant width=60%}
...
![Historical non-vegetation boundaries map.](/images/noplant_plt.jpeg){#fig-no-plant-map width=80%}
...
```bash
cd data510-fire
quarto render src/rf-clean.qmd
quarto render src/t-test.qmd
quarto render deliverables/M6-final-writeup/writeup.qmd
```
...
::: {#fig-species-maps layout-ncol=2}

```{r}
#| label: fig-pinus-map
#| fig-cap: "Ponderosa Pine."
#| echo: false
#| fig-width: 5
